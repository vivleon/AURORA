...
async def make_plan(user_input: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if model_runner:
        try:
            # [수정] 'schedule_find' (일정 찾기) 의도 추가
            categories = ["schedule_create", "schedule_find", "mail_send", "mail_compose", "search", "ocr", "summarize", "notes"]
    ...
    # [수정] 기존 'schedule'을 'schedule_create'로 명확히 분리
